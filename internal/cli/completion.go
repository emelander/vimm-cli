...
_vimm() {
  ...
  case "$cmd" in
    systems)
      ...
      ;;
    search)
      COMPREPLY=( $(compgen -W "--system --class --query --match --limit --offset --region --exclude-tags --no-header --help -h --json --plain --quiet -q --verbose -v --no-color --no-input --config --version" -- "$cur") )
      ;;
    download)
      COMPREPLY=( $(compgen -W "--system --query --match --all --id --output-dir --tmp-dir --concurrency -c --retries --retry-backoff --timeout --max-rps --resume --overwrite --dry-run --latest --revision --region --exclude-tags --verify --skip-verify --strict-hashes --count-check --strict-count --allow-mismatch --force --help -h --json --plain --quiet -q --verbose -v --no-color --no-input --config --version" -- "$cur") )
      ;;
    verify)
...
_vimm() {
  ...
  case $cmd in
    ...
    search)
      _arguments "--system[System code]" "--class[System class]" "--query[Search pattern]" "--match[Match mode]" "--limit[Limit]" "--offset[Offset]" "--region[Preferred region]" "--exclude-tags[Exclude tags]" "--no-header[Hide column headers]" "--help[Show help]" ;;
    download)
      _arguments "--system[System code]" "--query[Search pattern]" "--match[Match mode]" "--all[Download all]" "--id[Vault id]" "--output-dir[Output directory]" "--tmp-dir[Temp directory]" "-c[Concurrency]" "--concurrency[Concurrency]" "--retries[Retries]" "--retry-backoff[Retry strategy]" "--timeout[Timeout]" "--max-rps[Rate limit]" "--resume[Resume]" "--overwrite[Overwrite]" "--dry-run[Dry run]" "--latest[Prefer latest]" "--revision[Revision override]" "--region[Preferred region]" "--exclude-tags[Exclude tags]" "--verify[Verify hashes]" "--skip-verify[Skip verification]" "--strict-hashes[Strict hashes]" "--count-check[Count check]" "--strict-count[Strict count]" "--allow-mismatch[Allow mismatch]" "--force[Skip confirmation]" "--help[Show help]" ;;
    verify)
...
complete -c vimm -n '__fish_seen_subcommand_from search' -l exclude-tags -d 'Exclude tags'
...
complete -c vimm -n '__fish_seen_subcommand_from download' -l exclude-tags -d 'Exclude tags'
